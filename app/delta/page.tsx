...
// Default action buttons directive for Delta Airlines
const DEFAULT_DELTA_ACTION_BUTTONS: UiDirective = {
  type: "card",
  id: "delta-action-buttons",
  title: "How can I assist you today?",
  body: "Select an option to get started:"
};

export default function DeltaPage() {
  const [directives, setDirectives] = useState<UiDirective[]>([DEFAULT_DELTA_ACTION_BUTTONS]);
  const [captions, setCaptions] = useState<string[]>([
    "Hello! I'm your Delta Airlines Assistant. How can I help you with your travel today?",
  ...
  const [triggeredAction, setTriggeredAction] = useState<"book-flight" | "flight-check-in" | "flight-status" | "baggage-tracking" | null>(null);
  ...
  useEffect(() => {
    ...
    if (consent === "accepted") {
      ...
      setDirectives([DEFAULT_DELTA_ACTION_BUTTONS]);
      setCaptions([
        "Hello! I'm your Delta Airlines Assistant. How can I help you with your travel today?",
        "You can interact with me using voice or by clicking the buttons below."
      ]);
  ...
  const handleUserUtterance = async (text: string) => {
    ...
    try {
      ...
      const hasActionButtons = newDirectives.some(d => d.type === "card" && (d.id === "delta-action-buttons" || d.id === "action-buttons" || d.id === "doctor-action-buttons"));
      if (!hasActionButtons && !triggeredAction) {
        setDirectives([DEFAULT_DELTA_ACTION_BUTTONS, ...newDirectives]);
      } else {
      ...
      <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center max-w-md mx-auto p-8">
          ...
          <p className="text-blue-200/80 mb-8 text-lg">
            To use Delta Airlines Assistant, please accept our terms and enable microphone access.
          </p>
          <button
