...
// Default action buttons directive for Bank Concierge
const DEFAULT_BANK_ACTION_BUTTONS: UiDirective = {
  type: "card",
  id: "bank-action-buttons",
  title: "How can I assist you today?",
  body: "Select a banking service to get started:"
};

export default function BankPage() {
  const [directives, setDirectives] = useState<UiDirective[]>([DEFAULT_BANK_ACTION_BUTTONS]);
  const [captions, setCaptions] = useState<string[]>([
    "Hello! I'm your Bank Concierge. How can I help you with your banking needs today?",
  ...
  const [triggeredAction, setTriggeredAction] = useState<"account-balance" | "transfer-funds" | "bill-pay" | "transaction-history" | "loan-inquiry" | "deposit-check" | null>(null);
  ...
  useEffect(() => {
    ...
    if (consent === "accepted") {
      ...
      setDirectives([DEFAULT_BANK_ACTION_BUTTONS]);
      setCaptions([
        "Hello! I'm your Bank Concierge. How can I help you with your banking needs today?",
        "You can interact with me using voice or by clicking the buttons below."
      ]);
  ...
  const handleUserUtterance = async (text: string) => {
    ...
    try {
      ...
      const hasActionButtons = newDirectives.some(d => d.type === "card" && (d.id === "bank-action-buttons" || d.id === "action-buttons" || d.id === "doctor-action-buttons" || d.id === "delta-action-buttons"));
      if (!hasActionButtons && !triggeredAction) {
        setDirectives([DEFAULT_BANK_ACTION_BUTTONS, ...newDirectives]);
      } else {
  ...
  const handleActionClick = (action: "check-in" | "family-notifications" | "care-coordination" | "wellness-tracking" | "consultation" | "prescription" | "lab-results" | "medical-history" | "book-flight" | "flight-check-in" | "flight-status" | "baggage-tracking" | "account-balance" | "transfer-funds" | "bill-pay" | "transaction-history" | "loan-inquiry" | "deposit-check") => {
    // Only handle bank-specific actions, ignore others
    if (action === "account-balance" || action === "transfer-funds" || action === "bill-pay" || action === "transaction-history" || action === "loan-inquiry" || action === "deposit-check") {
      ...
      <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center max-w-md mx-auto p-8">
          ...
          <p className="text-blue-200/80 mb-8 text-lg">
            To use Bank Concierge, please accept our terms and enable microphone access.
          </p>
          <button
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <header className="relative z-10 p-6 flex items-center justify-between bg-white/5 backdrop-blur-xl border-b border-white/10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-blue-200/80">Bank Concierge • Powered by Human+</p>
          </div>
        </div>
      ...
      <footer className="relative z-10 mt-16 py-8 text-center text-sm text-blue-200/60 border-t border-white/5">
        <p>
          EverFriends MVP - Bank Concierge • Built with Next.js, TypeScript, and Tailwind CSS
        </p>
        {sessionId && (
